package com.ansari.distributed_lovable.workspace_service.consumer;

import com.ansari.distributed_lovable.common_lib.event.FileStoreRequestEvent;
import com.ansari.distributed_lovable.common_lib.event.FileStoreResponseEvent;
import com.ansari.distributed_lovable.workspace_service.entity.ProcessedEvent;
import com.ansari.distributed_lovable.workspace_service.repository.ProcessedEventRepository;
...
public class FileStorageConsumer {
    ...
    public void consumeFileEvent(FileStoreRequestEvent requestEvent){
        log.info("Received FileStoreRequestEvent for projectId: {}, filePath: {}", requestEvent.projectId(), requestEvent.filePath());

        if(processedEventRepository.existsById(requestEvent.sagaId())){
            log.info("Event with sagaId: {} has already been processed. Skipping.", requestEvent.sagaId());
        ...
        try{
            ...
            processedEventRepository.save(
                    new ProcessedEvent(requestEvent.sagaId(), LocalDateTime.now()
            ));

            sendResponse(requestEvent,true,null);
        }catch(Exception e){
            log.error("Error saving file for projectId: {}, filePath: {}. Error: {}", requestEvent.projectId(), requestEvent.filePath(), e.getMessage());
